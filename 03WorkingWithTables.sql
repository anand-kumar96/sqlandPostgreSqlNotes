...
----4 Primary and Foregin key  ==> all four realtionship always done by using foregin key
...
---6 create user table and fill data => Serial : Auto generate Id
  --> creating table
CREATE TABLE users(
  id INTEGER,
  username VARCHAR(50)
...
----> 7 Creating Photos table and fill data id ,url, user_id 
...
-- ==> Question
--: Create boat table and fill data
CREATE table boats (
  id serial PRIMARY KEY,
  name VARCHAR(100)
  );
